/**
 * @file linked_list_app.c
 *
 * @author Samah A. SHAYYA
 *
 * @brief This file contains code for the bidirectional list app.
 */
...
/**
 * @brief Handles interrupt signal (Ctrl + C) by deleting entire list prior to
...
void sigInterruptHandler(int sigNum) {
    ...
    puts("Deleted entire list, and now exiting...");
    exit(0);
}
...
int main() {
    ...
    // Getting list type (UNORDERED / ORDERED)
    ...
    do {
        ...
        displayCurrentList(root, recursive);

        /**
         * Identifying operation requested by the user excluding UPDATE in
         * the case of ordered list.
         */
        operationType = getOperationType(listType == ORDERED);

        // Performing requested operation
        performListOperation(&root, listType, operationType, recursive);

    } while (i < LIMIT); // It allows limted number of operations
